...
function Map({ userAddress }) {
    ...
    const imgUrlS = 'http://tmapapi.sktelecom.com/upload/tmap/marker/pin_r_m_s.png';

    const imgUrlE = 'http://tmapapi.sktelecom.com/upload/tmap/marker/pin_r_m_e.png';
    ...
    const initTmap = (nowPosition) => {
        const map = new window.Tmapv2.Map(
            ...
            {
                center: new window.Tmapv2.LatLng(nowPosition.latitude, nowPosition.longitude),
                zoom: 17,
            },
            []
        );
        ...
        const currentMarker = new window.Tmapv2.Marker({
            position: new window.Tmapv2.LatLng(nowPosition.latitude, nowPosition.longitude),
            icon: imgUrlS,
        ...
        try {
            ...
            if (resultCoordinate) {
                ...
                if (mapInstanceRef.current) {
                    ...
                    //마커
                    const currentMarker = new window.Tmapv2.Marker({
                        position: new window.Tmapv2.LatLng(currentMarkerPosition.latitude, currentMarkerPosition.longitude),
                        icon: imgUrlS,
                        iconSize: new window.Tmapv2.Size(30, 40),
    ...
    const centerMap = (position1, position2) => {
        if (mapInstanceRef.current && position1 && position2) {
            ...
            bounds.extend(new window.Tmapv2.LatLng(position1.latitude, position1.longitude));
            bounds.extend(new window.Tmapv2.LatLng(position2.latitude, position2.longitude));
    ...
    return (
        <div className="w-full">
            {loading && <Sktelecom className="w-full h-[25rem] lg:h-[30rem] drop-shadow-md" />}
            <div ref={mapContainerRef} id="TMapApp" className="w-full h-[25rem] lg:h-[30rem] drop-shadow-md"></div>
        </div>
    );
}
